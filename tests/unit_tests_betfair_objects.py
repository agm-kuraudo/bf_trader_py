...
class TestBetfairApp(unittest.TestCase):
    ...
    def test_call(self):
        ...
        self.assertEqual(
            my_call.call(Methods.GET, Urls.JSON_RPC_BET, {}).status_code, 400, "Expect a HTTP 400 response"
        )
    ...
    def test_betfair_objects(self):
        ...
        self.assertIsInstance(
            my_event_type.build_from_json("{'eventType': {'id': '1', 'name': 'Soccer'}, 'marketCount': 7672}"),
            EventType,
            "Event Type object not create via build from json",
        )

        self.assertEqual(
            my_event_type.build_from_json("{'eventType': {'id': '1', 'name': 'Soccer'}, 'marketCount': 7672}").name,
            "Soccer",
            "EventType created should be soccer",
        )

        # Competition Object - create from frame
        df, comp_list = my_comp.build_frame_from_json(
            '{"jsonrpc":"2.0","result":[{"competition":{"id":"12199359",'
        ...
        self.assertIsInstance(
            my_comp.build_from_json(
                "{'competition': {'id': '12199359', 'name': 'Premiership'}, 'marketCount': 144, 'competitionRegion': 'GBR'}"  # noqa: E501
            ),
            Competition,
            "Competition object not created via build from json",
        )
        ...
        self.assertEqual(
            ...
            "Premiership",
            "Competition created should be Premiership",
        )

        # Event object - create frame from json
        df, event_list = my_event.build_frame_from_json(
            '{"jsonrpc":"2.0","result":[{"event":{"id":"33589826","name":"Newcastle v Man City","countryCode":"GB","timezone":"GMT","openDate":"2024-09-28T11:30:00.000Z"},"marketCount":1},{"event":{"id":"33589827","name":"Nottm Forest v Fulham","countryCode":"GB","timezone":"GMT","openDate":"2024-09-28T14:00:00.000Z"},"marketCount":1}],"id":1}'  # noqa: E501
        )
        ...
        self.assertIsInstance(
            my_market.build_from_json(
                "{'marketId': '1.232498611', 'marketName': 'Match Odds', 'description': {'persistenceEnabled': True, 'bspMarket': True, 'marketTime': '2024-09-17T16:45:00.000Z', 'suspendTime': '2024-09-17T16:45:00.000Z', 'bettingType': 'ODDS', 'turnInPlayEnabled': True, 'marketType': 'MATCH_ODDS', 'regulator': 'MALTA LOTTERIES AND GAMBLING AUTHORITY', 'marketBaseRate': 5.0, 'discountAllowed': False, 'wallet': 'UK wallet', 'rules': '<!--Football - Match Odds --><br>Predict the result of this match.<br> All bets apply to Full Time according to the match officials, plus any stoppage time. Extra-time/penalty shoot-outs are not included.<br><br></b>For further information please see <a href=http://content.betfair.com/aboutus/content.asp?sWhichKey=Rules%20and%20Regulations#undefined.do style=color:0163ad; text-decoration: underline; target=_blank>Rules & Regs<br><br>\n', 'rulesHasDate': True, 'priceLadderDescription': {'type': 'CLASSIC'}}, 'totalMatched': 12303.74, 'runners': [{'selectionId': 65778, 'runnerName': 'Young Boys', 'handicap': 0.0, 'sortPriority': 1, 'metadata': {'runnerId': '65778'}}, {'selectionId': 63908, 'runnerName': 'Aston Villa', 'handicap': 0.0, 'sortPriority': 2, 'metadata': {'runnerId': '63908'}}, {'selectionId': 58805, 'runnerName': 'The Draw', 'handicap': 0.0, 'sortPriority': 3, 'metadata': {'runnerId': '58805'}}]}"  # noqa: E501
            ),
            Market,
            "Market object not created via build from json",
        )
        ...
        self.assertEqual(
            ...
            "Match Odds",
            "Market object created should be Match Odds",
        )
    ...
    def test_db_connection(self):
        ...
        self.assertTrue(
            db_details_string.get("host") is not None, "Host should be returned as part of db connection string"
        )
        self.assertTrue(
            db_details_string.get("port") is not None, "Host should be returned as part of db connection string"
        )
        self.assertTrue(
            db_details_string.get("db_name") is not None, "Host should be returned as part of db connection string"
        )
        ...
        self.assertEqual(
            len(db_connection.db_read(f"select * from bf.log_file where id='{db_connection.run_id}'")[0]),
            3,
            "Should be 3 columns returned",
        )
    ...
    def test_db_object_ids(self):
        ...
        first_update = my_db.db_read("select * from bf.betfair_object_ids where object_type='unit-test-type'")
        ...
        my_db.db_write_object_id(object_type="unit-test-type", object_name="name", object_id="12345")
        second_update = my_db.db_read("select * from bf.betfair_object_ids where object_type='unit-test-type'")
        ...
        third_update = my_db.db_read("select * from bf.betfair_object_ids where object_type='unit-test-type'")
