...
class Event(BetfairObject):
    ...
    def build_from_json(self, json):
        ...
        if not all(
            attr is not None for attr in [self.__marketCount, self.__timezone, self.__id, self.__name, self.__openDate]
        ):
        ...
        return Event(
            event_id=self.__id,
            name=self.__name,
            country_code=self.__countryCode,
            timezone=self.__timezone,
            open_date=self.__openDate,
            market_count=self.__marketCount,
        )
    ...
    def __str__(self):
        return (
            f"eventID: {self.__id}, eventName: {self.__name}, marketCount: {self.__marketCount}, "
            f"countryCode: {self.__countryCode}, timezone: {self.__timezone}, openDate: {self.__openDate}"
        )
